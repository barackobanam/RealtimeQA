...
class Room extends Controller {
    ...
    */
    public function __construct($options) {
        ...
        if ( array_key_exists($options[0], $this->actions ) ){
            ...
            $this->room_id = isset( $options[0] ) ? $options[0] : 0;
            if( $this->room_id == 0 ) {
   ...
   protected function output_presenter_controls(){
       if ( $this->is_presenter ) { 
           if ( !$this->is_active ) {
               ...
               $form_action = APP_URI . 'room/reopen';
           } else {
   ...
   protected function join_room(){
       ...
       // If the room exists, created the URL; otherwise, sends to a 404
